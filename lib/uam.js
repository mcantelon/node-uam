    ...
    if (registry[package] != undefined) {
      ...
      run.on('exit', function(code) {
        ...
        exports.getRegistry(function(err, userRegistry) {
          ...
          fs.writeFile(
            path.join(helpers.getUserHome(), '.uam'),
...
exports.runPackage = function(package, port) {
  ...
  run.stdout.on('data', function(data) {
    ...
    if (!running) {
      running = true;
      openTool = (process.platform == 'darwin') ? 'open' : 'gnome-open';
      openCommand = openTool + ' http://127.0.0.1:' + port
      exec(openCommand, {env: process.env});
    }
  });
...
exports.downloadNpmData = function(file, cb) {
  helpers.downloadFile(
    'http://registry.npmjs.org/-/by-field?field=uam',
    file,
    cb
  );
}
